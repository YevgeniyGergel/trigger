...
Card payments via LiqPay tied to sessions: per-psychologist merchant credentials, checkout links, webhook-driven payment statuses, fiscal receipts, and refunds on cancellation of paid sessions.
...
The system SHALL derive a session's default price from its service type (see `service-types`), snapshotting it into the session at creation time, and SHALL allow the psychologist to override the price of an individual session while its payment is not completed. Once a session is paid, its price MUST NOT change.

#### Scenario: Price derived from the service type
- **WHEN** a session is created from a service type with a price
- **THEN** the system snapshots that price into the session and uses it when generating the payment request
...
- **THEN** the system uses the overridden price when generating the payment request; the service type's catalog price is unchanged
